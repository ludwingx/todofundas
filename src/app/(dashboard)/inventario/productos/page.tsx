...
export default async function ProductsPage() {
  ...
  const products = productsData.map((product) => ({
    ...
    phoneModel:
      product.phoneModel?.status === "active" ? product.phoneModel : null,
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Stock (Bueno)</TableHead>
                ...
                {products.length > 0 ? (
                  products.map((product) => (
                    <TableRow key={product.id}>
                      ...
                      <TableCell>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span
                            ...
                            style={{
                              backgroundColor: product.color?.hexCode || "#000",
                            }}
                            title={product.color?.name || "Sin color"}
                          />
                      ...
                      <TableCell>
                        Bs.{" "}
                        {typeof product.priceRetail === "number"
          ...
          <div className="grid gap-4 md:grid-cols-5">
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
              <div className="text-2xl font-bold text-green-600">
                {products
                  .reduce(
                    (sum, product) =>
                      sum + (product.stock > 5 ? product.stock : 0),
                    0
                  )
                  .toLocaleString()}
              </div>
              <p className="text-xs text-muted-foreground">Unidades en stock sano</p>
            </div>
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
            ...
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
              ...
              <p className="text-xs text-muted-foreground">
                Productos sin stock sano
              </p>
            </div>
            <div className="rounded-lg border bg-red-100 bg-red-50 dark:bg-red-950/20 text-card-foreground shadow-sm p-4">
              <div className="text-2xl font-bold text-red-700">
                {products
                  .reduce((sum, product) => sum + (product.stockDamaged || 0), 0)
                  .toLocaleString()}
              </div>
              <p className="text-xs text-red-600 dark:text-red-400 font-medium">
                Unidades dañadas en almacén
              </p>
            </div>
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
              <div className="text-2xl font-bold">
                {products
                  .reduce((sum, product) => sum + product.stock + (product.stockDamaged || 0), 0)
                  .toLocaleString()}
              </div>
              <p className="text-xs text-muted-foreground">
